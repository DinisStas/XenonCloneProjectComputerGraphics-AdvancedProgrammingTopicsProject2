...
class LifePowerUp : public Character
{
public:
    //Wanted to make it a different way but for better implementation reused Die() and
    Player* player;

    LifePowerUp(Player* player, b2WorldId worldId, glm::vec2 position = { 0.0f, 0.0f },
        glm::vec2 scale = { 50.0f, 50.0f }, float rotation = 90.0f, float damage = 10.0f)
        :player(player), Character(worldId, "PlayerPowerup", 2003, "PlayerPowerUp", position, scale, rotation, damage)
    {
        b2Body_SetUserData(bodyId, this);
        LoadAnimation("XenonSprites/PUShield.bmp", 2, 4);
    }

    void Die() override {
        if (!player) return; // crash guard
        if (player->lifes <= 2) player->lifes++;
